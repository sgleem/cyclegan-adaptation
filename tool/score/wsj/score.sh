...
[ -x $score/wer_output_filter ] && ref_filtering_cmd="$score/wer_output_filter"
[ -x $score/wer_ref_filter ] && ref_filtering_cmd="$score/wer_ref_filter"
hyp_filtering_cmd="cat"
[ -x $score/wer_output_filter ] && hyp_filtering_cmd="$score/wer_output_filter"
[ -x $score/wer_hyp_filter ] && hyp_filtering_cmd="$score/wer_hyp_filter"
...
if [ $stage -le 1 ]; then
  ...
  done | ../best_wer.sh  >& $dir/scoring_kaldi/best_wer || exit 1
  ...
  if $stats; then
    ...
    $cmd $dir/scoring_kaldi/log/stats1.log \
      ...
      $score/wer_per_utt_details.pl --special-symbol "'***'" \| tee $dir/scoring_kaldi/wer_details/per_utt \|\
       $score/wer_per_spk_details.pl $data/utt2spk \> $dir/scoring_kaldi/wer_details/per_spk || exit 1;

    $cmd $dir/scoring_kaldi/log/stats2.log \
      cat $dir/scoring_kaldi/wer_details/per_utt \| \
      $score/wer_ops_details.pl --special-symbol "'***'" \| \
