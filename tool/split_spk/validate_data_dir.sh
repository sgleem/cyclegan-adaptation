#!/bin/bash
# Modified version of Kaldi script
# This script can run in DataStorageSplit package folder
# Require File
# 1. validate_text.pl
...
! cmp -s <(cat $data/utt2spk | awk '{print $1, $2;}') \
     <(${package_dir}/spk2utt_to_utt2spk.pl $data/spk2utt)  && \
...
if [ -f $data/text ]; then
  ${package_dir}/validate_text.pl $data/text || exit 1;
